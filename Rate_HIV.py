...
def Felsenstein (node, a, rate, alignment, cache, seq_indices, col):
    ...
    if key in cache:
        ...
        if alignment[seq_index, col] == a:
            return np.log(1)
        elif alignment[seq_index, col] == '-':          #All we need is these two extra lines apparently...
            return np.log(1)
        else:
            return np.NINF
    else:
...
if __name__ == "__main__":
    tree = utils.read_tree('data/HIV1_REF_2010_gp120.nwk')
    alignment = AlignIO.read('data/HIV1_REF_2010_gp120.fasta', "fasta")
...
# Implementing the algorithm for all bases at the node for all columns
    ...
    results = np.zeros((3,length, 2))
    for r in xrange(0,3):
        cache2 = {}
        for col in xrange(length-1, -1, -1):
    ...
    result_table = np.zeros((length,4))
    ...
    sheet1 = book.add_sheet("HIV Results")
    for col in xrange(0,length):
    	sheet1.write(col, 0, results[r,col,0])
    	
    book.save("results_HIV.xls")
